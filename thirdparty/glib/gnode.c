...
//#define g_node_alloc0()         g_slice_alloc0 (GNode)
//#define g_node_free(node)       g_slice_free (GNode, node)

gpointer
g_node_alloc0 ()
{
  gpointer mem = malloc (sizeof(GNode));
  if (mem)
    memset (mem, 0, sizeof(GNode));
  return mem;
}

void g_node_free(void * pointer)
{
    free(pointer);
}
		     ...
		     GNode *node)
{
  ...
  //g_return_val_if_fail (node != NULL, node);
  //g_return_val_if_fail (G_NODE_IS_ROOT (node), node);
  if (parent == NULL || node == NULL || !G_NODE_IS_ROOT (node))
  {
      return node;
  }
  if (sibling)
		 ...
		 gpointer	   data)
{
  ...
  if (root == NULL || func == NULL || order > G_LEVEL_ORDER || flags > G_TRAVERSE_MASK || (depth != -1 && depth <= 0))
  {
      return;
  }
	     ...
	     gpointer        data)
{
  ...
  if (root == NULL || order > G_LEVEL_ORDER || flags > G_TRAVERSE_MASK)
  {
      return NULL;
  }
		...
		GTraverseFlags  flags)
{
  ...
  if (root == NULL || flags > G_TRAVERSE_MASK)
  {
      return 0;
  }
		   ...
		   gpointer	   data)
{
  //g_return_val_if_fail (node != NULL, NULL);
  //g_return_val_if_fail (flags <= G_TRAVERSE_MASK, NULL);
  if (node == NULL || flags > G_TRAVERSE_MASK)
  {
      return NULL;
  }
			 ...
			 gpointer	   data)
{
  ...
  //g_return_if_fail (flags <= G_TRAVERSE_MASK);
  //g_return_if_fail (func != NULL);
  if (node == NULL || flags > G_TRAVERSE_MASK || func == NULL)
  {
      return;
  }
