...
/**
 * @brief READ AN INTEGER ONE BYTE AT A TIME.  Four bytes are read, ordered from
...
int intio_read(FILE *file)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (fread(&b[i], 1, 1, file) != 1) return 0;
    }
...
/** 
 * @brief WRITE AN INTEGER ONE BYTE AT A TIME.  Four bytes are written, ordered from
 * low to high order.  These are considered to represent a signed integer,
...
void intio_write(FILE *file, int value)
{
    unsigned char b;
    int i;
