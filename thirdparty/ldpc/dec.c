...
static void initprp(mod2sparse *H, double *lratio, char *dblk, double *bprb);
static void iterprp(mod2sparse *H, double *lratio, char *dblk, double *bprb);
 ...
 * @param dblk       Place to stored decoded message
 * @param bitpr      Place to store marginal bit probabilities
 * @param max_block  Maximize probability of whole block being correct?
...
unsigned ldpc_decode_enum(double *lratio, char *dblk, double *bitpr, int max_block, generator_matrix* G)
{
    ...
    if (bpr != 0 && bpr != bitpr) boxing_memory_free(bpr);
    boxing_memory_free(cblk);
    boxing_memory_free(lk0);
    boxing_memory_free(lk1);
...
/**
 * @brief INITIALIZE PROBABILITY PROPAGATION.  Stores initial ratios, probabilities,
 * and guess at decoding. 
 *
 * @param H      Parity check matrix
 * @param ratio  Likelihood ratios for bits
 * @param dblk   Place to store decoding
 * @param bprb   Place to store bit probabilities, 0 if not wanted
 */
...
/**
 * @brief DO ONE ITERATION OF PROBABILITY PROPAGATION. 
 * H        Parity check matrix
 * lratio   Likelihood ratios for bits
 * dblk     Place to store decoding
