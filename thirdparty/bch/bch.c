...
static const unsigned int prim_poly_tab[] = {
    ...
    BCH_PRIM_POLY_0x409,
    BCH_PRIM_POLY_0x805,
    BCH_PRIM_POLY_0x1053,
    BCH_PRIM_POLY_0x201B,
    BCH_PRIM_POLY_0x402B,
    BCH_PRIM_POLY_0x8003
};

static int bch_fls(int x)
{
    int r = 32;
...
#define GF_POLY_SZ(_d) (sizeof(struct gf_poly)+((_d))*sizeof(unsigned int))

/* polynomial of degree 1 */
struct gf_poly_deg1 {
    unsigned int    deg;  /* polynomial degree */
	unsigned int   c[2];  /* polynomial terms */
};
		...
		unsigned int len, uint8_t *ecc)
{
	...
	if (ecc) {
        ...
        memset(bch->ecc_buf, 0, sizeof(*r)*(l + 1));
	}
	...
	data += 4*mlen;
	len  -= 4*mlen;
    memcpy(r, bch->ecc_buf, sizeof(*r)*(l + 1));
    ...
    memcpy(bch->ecc_buf, r, sizeof(*r)*(l + 1));
